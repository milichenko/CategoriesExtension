...
unsigned char   cnv(unsigned char b)
{
...
void   CharToHex(unsigned char c, unsigned char *buff)
{
...
{
    if(buff == NULL || size == 0 || out_buff == NULL) return NULL;
    char *    In     = ( char *)buff;
    unsigned int    cn = 0,cc = 0;
    for(cc=0;cc<size;cc++)
    {
        CharToHex(*In,(unsigned char*)(out_buff+cn));
        In++;
        cn+=2;
    }
    return out_buff;
...
NSString *md5_ns(NSString *imput)
{
    ...
    unsigned char _str[36];
    memset(_str,0,sizeof(_str));
    md5_calc((unsigned char *)_bytes,(unsigned char *) [imput UTF8String], [imput length]);
    return [[NSString alloc] initWithUTF8String:(const char*)BuffToHex((void*)&_bytes[0], 16,(char*)&_str[0])];
